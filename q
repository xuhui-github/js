...
DDEESSCCRRIIPPTTIIOONN
       ...
       will be highlighted with an asterisk. Option -r causes the remote-tracking branches to be listed, and option -a
       shows both local and remote branches. If a <pattern> is given, it is used as a shell wildcard to restrict the
       output to matching branches. If multiple patterns are given, a branch is shown if it matches any of the
       ...
       _H_E_A_D (i.e. the tip of the current branch).

       The command’s second form creates a new branch head named <branchname> which points to the current _H_E_A_D, or
       ...
       branch.<name>.remote and branch.<name>.merge configuration entries) so that _g_i_t _p_u_l_l will appropriately merge
       ...
       remote-tracking branches if they no longer exist in the remote repository or if _g_i_t _f_e_t_c_h was configured not to
       fetch them again. See also the _p_r_u_n_e subcommand of ggiitt--rreemmoottee(1) for a way to clean up all obsolete
...
OOPPTTIIOONNSS
       ...
       -f, --force
           Reset <branchname> to <startpoint> if <branchname> exists already. Without -f _g_i_t _b_r_a_n_c_h refuses to change
           an existing branch. In combination with -d (or --delete), allow deleting the branch irrespective of its
           merged status. In combination with -m (or --move), allow renaming the branch even if the new branch name
           already exists.
       ...
       --column[=<options>], --no-column
           ...
           --no-column without options are equivalent to _a_l_w_a_y_s and _n_e_v_e_r respectively.
       ...
       -t, --track
           ...
           mark the start-point branch as "upstream" from the new branch. This configuration will tell git to show the
           relationship between the two branches in git status and git branch -v. Furthermore, it directs git pull
           ...
           behave as if _-_-_n_o_-_t_r_a_c_k were given. Set it to always if you want this behavior when the start-point is
       ...
       <branchname>
           The name of the branch to create or delete. The new branch name must pass all checks defined by ggiitt--cchheecckk--
           rreeff--ffoorrmmaatt(1). Some of these checks may restrict the characters allowed in a branch name.
       ...
       --sort=<key>
           ...
           --sort=<key> option multiple times, in which case the last key becomes the primary key. The keys supported
           are the same as those in git for-each-ref. Sort order defaults to sorting based on the full refname
           (including refs/...  prefix). This lists detached HEAD (if present) first, then local branches and finally
           remote-tracking branches.
...
EEXXAAMMPPLLEESS
       ...
       Delete an unneeded branch
           ...
           11.. Delete the remote-tracking branches "todo", "html" and "man". The next _f_e_t_c_h or _p_u_l_l will create them
           again unless you configure them not to. See ggiitt--ffeettcchh(1).
           22.. Delete the "test" branch even if the "master" branch (or whichever branch is currently checked out) does
...
NNOOTTEESS
       ...
       ·   --merged is used to find all branches which can be safely deleted, since those branches are fully contained
           by HEAD.
...
SSEEEE AALLSSOO
       ggiitt--cchheecckk--rreeff--ffoorrmmaatt(1), ggiitt--ffeettcchh(1), ggiitt--rreemmoottee(1), ““UUnnddeerrssttaannddiinngg hhiissttoorryy:: WWhhaatt iiss aa bbrraanncchh??””[1] in the Git
       User’s Manual.
...
Git 2.7.4                                             04/20/2020                                         GIT-BRANCH(1)
